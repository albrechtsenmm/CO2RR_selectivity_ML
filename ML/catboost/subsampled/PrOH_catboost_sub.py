...
df = pd.read_csv("../../../Data/Datasets/BSc/data_class_gaussian", index_col=False)
...
def train(X, Y):
    ...
    for i in range(n_samples):
        ...
        # Storing the result
        test_results.append(
            (df["metal_facet"][i], test_pred[0], y_test[0], test_pred[1], rmse)
        )

    rmse_mean = np.mean(np.abs([t[4] for t in test_results]))
