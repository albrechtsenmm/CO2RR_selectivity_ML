...
db = connect('isolated_molecules_bag.db')
...
calc2 = GPAW(h=0.18,
            xc='BEEF-vdW',
            basis='szp(dzp)',
            convergence={'eigenstates':1,'density':5.0e-6},
            mixer=Mixer(0.05, 3, weight=100),
...
for molecule in molecules:
    ...
    molecule.calc = calc2
    dyn = BFGS(molecule, trajectory=f'{molecule.symbols}_isolated_bag.traj')
    dyn.run(fmax=0.05)
